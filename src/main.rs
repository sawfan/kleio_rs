...
fn run_wikidata_truthy(args: Vec<String>) -> Result<(), String> {
    ...
    let report = run_truthy_import(&options).map_err(|err| {
        format!(
    ...
    eprintln!(
        "kleio: wrote {} relevant Wikidata facts from {} lines to {} (humans_detected={})",
...
fn run_wikidata_closure(args: Vec<String>) -> Result<(), String> {
    ...
    let report = run_truthy_closure_import(&options).map_err(|err| {
        format!(
    ...
    eprintln!(
        "kleio: wrote {} one-hop closure facts from {} lines to {} (seed_qids={}, seed_subjects_seen={}, humans_detected={})",
        report.facts_written,
        report.lines_read,
        report.output_path.display(),
        report.seed_qids,
...
fn run_wikidata_label_seeds(args: Vec<String>) -> Result<(), String> {
    ...
    eprintln!(
        "kleio: wrote {count} label seed QIDs from {} to {}",
        input_path.display(),
        output_path.display()
    );
    Ok(())
...
fn run_wikidata_drafts(args: Vec<String>) -> Result<(), String> {
    ...
    eprintln!(
        "kleio: wrote {} Wikidata person drafts from {} facts to {} (humans_written={}, labels_loaded={})",
...
fn run_wikidata_kleio(args: Vec<String>) -> Result<(), String> {
    ...
    eprintln!(
        "kleio: wrote archive {} from {} drafts (people={}, events={}, families={}, places={})",
        report.output_path.display(),
        report.drafts_read,
...
fn run_wikidata_kleio_inspect(args: Vec<String>) -> Result<(), String> {
    ...
    eprintln!(
        "kleio: inspected {} (people={}, events={}, families={}, places={}, notes={})",
        report.path.display(),
        report.people,
        report.events,
        report.families,
...
fn print_wikidata_label_seeds_help() {
    println!(
        "kleio import wikidata-label-seeds\n\n\
...
fn print_wikidata_kleio_inspect_help() {
    println!(
        "kleio import wikidata-kleio-inspect\n\n\
